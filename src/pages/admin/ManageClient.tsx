...
export default function ManageClient(): JSX.Element {
  const [selected, setSelected] = useState<number[]>([1]); // Dustin is selected by default
  ...
  const handleSelectAll = () => {
    if (selected.length === clients.length) {
      ...
      setSelected(clients.map((_, idx) => idx));
    }
  };
